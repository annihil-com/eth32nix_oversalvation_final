...
void xcmd_RandMac()
{
	...
	Syscall.UI_Print("^3New Jaymac: ^7%s\n^dSetting won't take effect until next map (or do /reconnect)!\n", eth32.settings.jayMac);
}

void xcmd_unbanCommand()
{
	system("rm -rf ~/.etwolf/etmain/etkey");
	Syscall.UI_ExecuteText("eth_randguid;; wait 50\n");
	Syscall.UI_ExecuteText("pb_cdkeyreg; wait 500\n");
	Syscall.UI_ExecuteText("eth_randmac; wait 50; eth_randname; reconnect\n");
}

void xcmd_HackSpam()
{	// sol: added simple spam, feel free to modify however you like
	Syscall.UI_ExecuteText("say \"^0[^2ETH32^3nix ^0- ^5SALVATION ^0- ^3By rabbnix^0]\n\"\n");
	Syscall.UI_ExecuteText("say \"^0[^3An Enemy Territory Hack for Linux^0]\n\"\n");
	Syscall.UI_ExecuteText("say \"^0[^3Cheat Undetected ^0- ^3Cheat FREELY ^0- ^3Cheat OpenSource^0]\n\"\n");	
}
...
void setEtproOS()
{
	...
	Syscall.UI_Print("^3new etpro OS set to %s", eth32.settings.etproOs ? "win32" : "linux");
	free(cmd);
...
void CXCommands::RegisterCommands()
{
	...
	orig_Cmd_AddCommand("eth_spoofOs", &setEtproOS);
